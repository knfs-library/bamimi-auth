...
class JWT {
	...
	*/
	async generateAccessToken(data) {
		...
		if (this.config.refreshToken && this.config.refreshToken.use && (!data.refreshToken || "" === !data.refreshToken)) {
	...
	async verify(token, isRefreshToken = false) {
		...
		return await jwt.verify(token, secretKey).then(async (payload) => {
			if (!payload) {
				return false;
			}
...
export { JWT, configType, storageTypeEnum };
